...
from github_actions_utils.messaging_consumer import Message
...
def test_produce_message(kafka_broker, shared_data):
  ...
  produce_message(CONFIG.bootstrap_servers, "redpanda", shared_data)
...
def consume_message(bootstrap_servers, topic=CONFIG.topic, offset=None,
    partition=0) -> Dict:
  ...
  try:
    ...
    print(Message(
        payload={"message": f'''Received message: {msg.value().decode('utf-8')},
      offset {msg.offset()}
      topic {msg.topic()}
